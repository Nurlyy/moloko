...
class ConnectionsController extends \app\base\Controller
{
    ...
    public function actionReadAllGuests()
    {
        $this->guestManager->readAllGuests(Yii::$app->user->id);
    }


    public function actionReadAllLikes()
    {
        $type=isset($_GET['type'])?$_GET['type']:$LikeManager::TYPE_FROM_CURRENT_USER;
        $types = [LikeManager::TYPE_FROM_CURRENT_USER, LikeManager::TYPE_TO_CURRENT_USER, LikeManager::TYPE_MUTUAL];
        if (!in_array($type, $types)) {
            throw new BadRequestHttpException();
        }
        return $this->likeManager->readLikes($type, Yii::$app->user->id);
    }
    ...
    public function actionGetguestlikecount(){
        $likes = Like::find()->where(['to_user_id' => Yii::$app->user->id, 'is_read' => 0])->count();
        $guests = Guest::find()->where(['visited_user_id' => Yii::$app->user->id, 'is_read' => 0])->count();
        // return Like::find()->andWhere(['to_user_id' => $id, 'is_read' => 0])->count();
        // $likes = Like::getlikecount(Yii::$app->user->id);
        // $guests = Guest::getguestcount(Yii::$app->user->id);
        return $likes + $guests;
        // return $likes;
    }
