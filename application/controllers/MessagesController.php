...
class MessagesController extends \app\base\Controller
{
    ...
    public function actionMessages($contactId)
    {
        ...
        $age = 0;
        if($contact->profile !== null) { $age = $contact->profile->getAge(); } else { $age = 18; }
        $prefix = $this->YearTextArg($age);
        ...
        return $this->sendJson([
            ...
            'contact' => [
                ...
                'age' => $age . " " . $prefix,
                'location' => $contact->profile !== null ? $contact->profile->getDisplayLocation() : '',
            ],
            'messages' => $messages,
        ]);
    }
