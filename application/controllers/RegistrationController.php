...
class RegistrationController extends \app\base\Controller
{
    ...
    public function actionRegister()
    {
        ...
        if ($this->request->post()) {
            ...
            $post['register-form']['username'] = mt_rand(1, 100000) . date('mdYHis');
            $date = $post['register-form']['dobYear'] . '-' . $post['register-form']['dobMonth'] . '-' . $post['register-form']['dobDay'];
            $post['register-form']['dob'] =  $date;
            $post['register-form']['looking_for_sex'] = ($post['register-form']['sex'] == 1) ? 2 : 1;
            if ($model->load($post) && $model->create()) {
                if ($model->sex == 2) {
                    $model->getUser()->premium->activatePremium($model->getUser()->id, 1000);
                }
    ...
    public function actionConnect($code)
    {
        ...
        if ($model->load($this->request->post()) && $model->create(true)) {
            if ($model->sex == 2) {
                $model->getUser()->premium->activatePremium($model->getUser()->id, 10);
            }
