...
class SettingsController extends \app\base\Controller
{
    ...
    public function actionProfile()
    {
        ...
        if ($this->request->post('Profile')) {
            ...
            $mod = ProfileExtraForm::createFromFields(
            ...
            if ($this->request->post('Profile')) {
                ...
                $date = $this->request->post('Profile')['dobYear'] . '-' . $this->request->post('Profile')['dobMonth'] . '-' . $this->request->post('Profile')['dobDay'];
                $postData['sex'] = $model->sex;
                $postData['country'] = null;
                $postData['looking_for_from_age'] = null;
                $postData['looking_for_to_age'] = null;
                $postData['looking_for_sex'] = $model->looking_for_sex;
                $postData['Profile']['dob'] = $date;
            }

            if ($model->load($postData) && $mod->load($this->request->post()) && $mod->validate()) {
                $model->save();
                foreach ($mod->getAttributes() as $attribute => $value) {
        ...
        return $this->render('profile', [
            ...
            'category' => $profileFieldCategories[0],
