...
class LoginForm extends \yii\base\Model
{
     ...
     * EDITED BY NURLYTAN: gives premium for all women for 1000 days every time they log in.
     *
    ...
    public function login()
    {
        if ($this->validate() && $this->user) {
            // var_dump($this->user->premium->getIsPremium());exit;
            $isLogged = Yii::$app->getUser()->login($this->user, $this->rememberMe ? $this->rememberFor : 0);

            if ($isLogged) {
                $this->user->updateAttributes(['last_login_at' => time()]);
                
                if($this->user->profile->sex == 2){
                   $premium = UserPremium::activatePremium($this->user->id, 1000);
                }
