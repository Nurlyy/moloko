...
<div class="conteaner-profile" data-user-id="<?= $user->id ?>">
    <div class="conteaner-header-profile">
        <button onclick="history.back();">Назад</button>
    </div>
    <div class="conteaner-content-profile">
        <div class="conteaner-photo">
            <?= ProfilePhotos::widget([
                ...
                'wrapperOptions' => [
                    'class' => 'photo-wrapper d-flex justify-content border-style',
                ],
                'photoOptions' => [
                    'class' => 'gallery-item profile-photo-item pad-a',
                    'width' => 137,
                    'height' => 137,
                ],
                'firstPhotoOptions' => [
                    'class' => 'gallery-item profile-photo-item profile-photo-main',
                    'width' => 423,
                    'height' => 423,
                ],
                'labelOptions' => [
        ...
        <div class="contenaer-info-user">
            <div class="wrapper">
                <?php if (!Yii::$app->user->isGuest): ?>
                    <?php if (Yii::$app->user->id == $user->id): ?>
                    <?php else: ?>
                        <div class="wrapper-conteaner-block-user">
                            <div class="conteaner-block-user">
                                <?php \yii\widgets\Pjax::begin(['id' => 'pjax-profile-actions', 'options' => ['tag' => 'span']]) ?>
                                    <button type="button" data-toggle="dropdown" class="btn-block-user">
                                        <?= Icon::fe('more-horizontal') ?>
                                    </button>
                                    <div class="dropdown-menu dropdown-menu-right dropdown-block">
                                        <button href="#" data-toggle="modal" data-target="#profile-report" class="items-block-menu">
                                            <?= Yii::t('youdate', 'Report') ?>
                                        </button>
                                        <button class="btn-ajax items-block-menu" data-action="<?= Url::to(['/block/toggle']) ?>"
                ...
                <?php endif; ?>
                <div class="name">
                    <?= Html::encode($profile->getDisplayName()) ?>, <?= $profile->getAge() ?>, <?= $profile->getDisplayLocation() ?>
                </div>
                <div>
                    <?= $this->render('/partials/online-status-profile', ['model' => $user]) ?>
                </div>
                <div class="conteaner-nav-user">
                    <?php if (!Yii::$app->user->isGuest && Yii::$app->user->id !== $user->id): ?>
                        <button class="btn-direct" data-toggle="modal" data-target="#profile-new-message">
                            Написать сообщение
                        </button>
                        <button class="btn-like btn-like-toggle <?= $likeByCurrentUser == null ? 'btn-not-liked' : 'btn-liked' ?>"
                            rel="tooltip"
                    ...
                    <?php if (!Yii::$app->user->isGuest): ?>
                    <?php if (Yii::$app->user->id == $user->id): ?>
                        <a href="<?= Url::to(['/settings/profile']) ?>" class="btn-editing">
                            <?= Icon::fe('edit', ['class' => 'mr-2']) ?>
                            <?= Yii::t('youdate', 'Редактировать анкету') ?>
                        </a>
                    <?php else: ?>
                    <?php endif; ?>
                <?php else: ?>
