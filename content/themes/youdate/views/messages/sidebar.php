...
?>
<div class="messages-conversations-filter">
    <p class="dialogue">Мои диалоги</p>
        ...
        <div class="conversations list-group list-group-flush">
            <a href="#"
               ...
               class="conversation list-group-item list-group-item-action flex-column align-items-start
                ...
                {{item.contact.id == currentContactId ? 'active-item' : ''}}">
                ...
                <span class="conteaner-contact-name">
                    <span class="name">
                        <!-- {{ item.contact }} -->
                        {{ item.contact.full_name }}, {{ item.contact.age }}, <span class="location">{{ item.contact.location }}</span>
                    </span>
                    <span class="last-message">
                        <span class="{{ newMessagesCounters[item.contact.id].new_messages_count >= '1' ? 'last-message-text' : ''}}">
                            {{ item.last_message.text }}
                        </span>
                    </span>
                    <span ng-show="newMessagesCounters[item.contact.id]">
                        <span class="new-counters-messages">
                            {{ (newMessagesCounters[item.contact.id].new_messages_count > 99) ? '99+' : newMessagesCounters[item.contact.id].new_messages_count }}
                        </span>
                    </span>
                </span>
