var updateintervalcounter = false;
function startcounter(){
    if(window.location.href.indexOf("messages") < 0 ){
        // updateIntervalId = true;
        $.ajax({
            url: 'connections/getguestlikecount',
            method: "GET",
            success: function(data){
                console.log(data);
            }
        });
        setInterval(function () { getcount(); getNotifications(); }, 5000);
    }
}

function stopcounter(){
    clearInterval(updateintervalcounter);
    updateintervalcounter = false;
}

function getcount(){
    $.ajax({
        url: appBaseUrl() + 'messages/conversations',
        method: "GET",
        success: function(response){
            if(response.success){
                c=0;
                for(var i in response.newMessagesCounters){
                    c+=parseInt(response.newMessagesCounters[i].new_messages_count);
                }
                if(c>0){$("#counterMessage").css("display","block");console.log("newmessage");$("#counterMessage").text((c>99)?"99+":c);} else {$("#counterMessage").css("display","none");}
            }
        },
    })
}
...
function getNotifications(){    
    $.ajax({
        url: appBaseUrl() + 'connections/getguestlikecount',
        method: "GET",
        success: function(data){
            if(data > 0){
                $("#notificationCounter").css("display","block");
                $("#notificationCounter").text((data>99)?"99+":data);
            }
            else {
...
function checkParams() {
    var radio = $('#radio-1').val();
    var name = $('#register-form-name').val();
    var dobday = $('#register_form_dobday').val();
    var dobMonth = $('#register_form_dobMonth').val();
    var dobYear = $('#register_form_dobYear').val();
    var email = $('#register-form-email').val();
    var city = $('#register_form_city').val();
    var password = $('#register-form-password').val();
    if (name.length == 0 ||name.length == 0 || dobday.length == 0 || dobMonth.length == 0 || dobYear.length == 0 || email.length == 0 || city.length == 0 || password.length == 0 ) {
        $('#register-form-submit').attr('disabled', 'disabled').addClass('btn-disabled')
    }
    else {
        $('#register-form-submit').removeAttr('disabled').removeClass('btn-disabled');
    }
}
