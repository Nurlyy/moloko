...
app.controller('MessagesController', ['$scope', '$http', 'Upload', function($scope, $http, Upload) {
    ...
    $scope.loggedUser = {
        id: $('.messenger').attr('data-user-id'),
        avatar: $('.messenger').attr('data-user-avatar'),
        fullName: $('.messenger').attr('data-user-fullname')
    };
    ...
    $scope.setCurrentContactId = function(id, key) {
        ...
        $(".contenaer-conversation-header").css("display", "none");
        ...
        if($scope.currentContactId === null){
            $(".items").css("display", "block");
            $(".contenaer-conversation-header").css("display", "block");
        ...
        return $http({ url: appBaseUrl() + 'messages/conversations', params: { query: $scope.conversationsQuery } })
            .then(function(response) {
                if (response.status === 200 && response.data.success) {
                    ...
                    c=0;
                    for(var i in $scope.newMessagesCounters){
                        c+=parseInt($scope.newMessagesCounters[i].new_messages_count);
                    }
                    if(c>0){$("#counterMessage").css("display","block");$("#counterMessage").text((c>99)?"99+":c);} else {$("#counterMessage").css("display","none");}
                    
                }
                if (!$scope.initialStateLoaded || resetCurrentContact === false) {
    ...
    $scope.resetCurrentContact = function() {
        if (Object.keys($scope.conversations).length) {
            ...
            $scope.setCurrentContactId(null);
            $scope.currentContact = null;
        }
    };
    ...
    $scope.readCurrentConversation = function () {
        ...
        for (var i in $scope.messages) {
            if($scope.messages[i].is_new) news++;
        }
        if(($scope.messages[Object.keys($scope.messages)[Object.keys($scope.messages).length - 1]].is_new) && ($scope.messages[Object.keys($scope.messages)[Object.keys($scope.messages).length - 1]].to_user_id == $scope.loggedUser.id)){
            if((!(parseInt($("#counterMessage").text())-news) > 0 || $("#counterMessage").text() == '')){
                $("#counterMessage").css("display","none");
            }
        }
    ...
    $scope.getMessages = function(isMessenger = false) {
        $http.get(appBaseUrl() + 'messages/messages', {
            ...
            if (response.status === 200 && response.data.success) {
                ...
                if(isMessenger){$(".items").css("display", "block");
                $(".contenaer-conversation-header").css("display", "block");
                $(".loading").css('display', 'none');}
            }
